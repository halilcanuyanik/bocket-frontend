...
function prepareEventSeatMap(event) {
  if (!event?.eventSeatMap) return null;

  const currencySymbol = formatCurrency(event?.pricing?.currency);
  const seatMapCopy = JSON.parse(JSON.stringify(event.eventSeatMap));

  seatMapCopy.groups = seatMapCopy.groups.map((group) => {
    const finalPrice = group.price ?? event?.pricing?.base ?? null;
    return {
      ...group,
...
function GroupLabel({ groupId, price }) {
  return (
    <div className="absolute -top-10 w-full text-center text-gray-500 text-xs font-bold uppercase tracking-wider">
      {groupId}
      {price && (
        <div className="font-bold text-md text-lively-orange">{price}</div>
      )}
    </div>
  );
}
